...
const FriendProfileForumPosts = ({ userId }) => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        const postsResponse = await axios.get(
          "http://localhost:3000/api/forum/friends",
          {
        ...
        // Filter posts by the friend's userId
  ...
  return (
    <>
      <div className="p-2 md:p-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Objave</h1>
        <div className="space-y-4">
          {posts.length === 0 ? (
            <p className="text-center text-gray-400 italic py-8">Ni objav.</p>
          ) : (
            posts.map((post) => (
              <ForumPagePost
                key={post._id}
...
export default FriendProfileForumPosts;
